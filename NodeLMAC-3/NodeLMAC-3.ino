...
//  Serial.println(nodeGetString);
  if(nodeGetString[0] == char(ID + 48)){
    nodeHandleServerLMAC3(nodeGetString); //处理服务器回传数据
  }
...
void nodeSendToServer(String sensorData){
  ...
  if(firstSend == 0){
    ...
    mySerial.print("+++");
    delay(50);
    mySerial.println("AT+TSF=0"+SF[bestSF]);
    delay(50);
...
//服务器回传数据处理，更新本地信道占用矩阵 函数(LMAC-3版本)
//LMAC-3要基于LMAC-2，因为TCP连接必须由节点发起请求，服务器响应；
//所以如果维护本地信道占用矩阵仅依赖服务器的数据是不够的，在繁忙时可能服务器来不及响应，导致本地一直无法更新矩阵，因此本地也得行动起来，原理就是LMAC-2，当收到有来自服务器的数据（收到回复）则直接使用服务器数据
//服务器回传数据的格式为:节点ID+"ANSWERED"+ CH/SF + 4个数据，每个数据对应的是矩阵对应位置代表的CH/SF信道的拥挤等级（服务器广播）+ 时间(发送数据的时间）
void nodeHandleServerLMAC3(String serverData){
  if(int(serverData[0] - 48) == ID){
    ...
    unsigned long time1 = serverData.substring(15,serverData.length()).toInt();
...
//    }
    ...
    //使用服务器的矩阵数据更新本地矩阵
    int count = 11; //count = 0是nodeID
    for(int i = 0; i < 2; i++){
      for(int j = 0; j < 2; j++){
        chsfMatrix[i][j] = int(serverData[count] - 48); // char转int直接减去48就行
        count = count + 1;
      }
    }
  }
